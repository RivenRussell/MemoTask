...
当前发布版本为 **v5.1.0**。v5.0.1 修复标签、AI 整理结果和同步状态的数据链路；v5.0.2 优化刷新反馈和主要操作忙碌状态；v5.0.3 改善 Todo 勾选、完成归档和标签录入体验；v5.0.4 将标签改为结构化数据并补充彩色标签 chip；v5.1.0 是 GitHub main 分支和安装包发布版本。
...
- 刷新失败时不再显示“已刷新”，避免弱网或接口错误时给出误导反馈。
- 刷新过程中会拦截重复刷新触发，并在页面顶部显示细进度条。
- 发布、AI 整理、保存、归档、Todo 编辑和设置操作增加更明确的进行中文案与禁用状态。
- 为按钮、通知、卡片和刷新反馈补充克制过渡动画，并兼容 `prefers-reduced-motion`。

v5.0.3 体验优化：

- Todo 勾选改为单项 pending 状态，后台刷新历史和同步状态时不再锁住整张 Memo 详情卡。
- 服务端返回 Memo 已完成归档时，前端会立即从当前队列移除并插入历史列表。
- 草稿输入区和 Memo 展开编辑态的标签入口会真实写入 `#标签` 文本，发布或保存后由后端解析入库。

v5.0.4 标签体验优化：

- 草稿输入区和 Memo 展开编辑态的标签不再写进正文，而是作为 `tags` 数组随发布、保存请求提交。
- 后端优先使用结构化 `tags` 写入 `memo_tags`，未传 `tags` 的旧客户端和旧正文仍兼容 `#标签` 解析。
- 标签 chip 增加稳定的自动配色，同一个标签在侧边栏、卡片和编辑区会显示一致的色彩。
...
```text
release/electron-v5.1.0/MemoTask-5.1.0-x64.exe
```
...
```text
release/electron-v5.1.0/MemoTask-5.1.0-android.apk
```
...
v5.1.0                  GitHub main 发布版本，包含 v5.0.4 标签体验优化与 PC/Android 安装包
```
