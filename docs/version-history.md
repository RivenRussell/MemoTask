...
v2.0.0 是账号管理版本。它包含注册、登录、邮箱验证、密码找回、重置密码、会话管理，以及按账号隔离 Memo 数据。
...
当前远端 `main` 与本地 v2 开发线不是同一条直接历史。为了避免覆盖远端主分支，建议先推送当前分支和标签：
...
后续如果要让 GitHub 默认首页直接显示 v2，可以在 GitHub 上创建 Pull Request，或在确认无风险后再合并到 `main`。
